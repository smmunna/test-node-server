 ...
 */
const saveFile = (fileBuffer, originalName, folderPath, req) => {
    // Ensure the folder path exists
    const uploadPath = path_1.default.join(process.cwd(), folderPath);
    if (!fs_1.default.existsSync(uploadPath)) {
        fs_1.default.mkdirSync(uploadPath, { recursive: true });
    }
    // Create a unique filename
    const uniqueFilename = `user-${Date.now()}${path_1.default.extname(originalName)}`;
